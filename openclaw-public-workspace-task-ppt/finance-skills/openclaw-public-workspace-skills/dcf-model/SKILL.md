...
**🚨 TRAINING DATA IS OUTDATED - ALWAYS USE LIVE DATA 🚨**

**BEFORE STARTING:**
1. **Check today's date** - DCF models must use the most recent financial data
2. **Source historical financials**:
   - **MCP sources** (if available): Daloopa, FactSet, S&P Kensho
   - **FREE alternatives** ✅:
     - **Brave API + web_search** - 已配置，搜索最新财报数据
     - **SEC EDGAR** - 免费下载 10-K/10-Q (https://www.sec.gov/edgar)
     - **公司官网 Investor Relations** - 季度财报、业绩演示文稿
     - **Yahoo Finance / Google Finance** - 免费股价、市值、Beta
3. **Use 最新季度优先，其次 FY2025 or latest quarterly results** for base year projections
4. **Verify market data freshness** - WACC inputs (risk-free rate, beta, market premium) must be current

**Data Requirements:**
- [ ] Historical financials from FY2020-2024 (or latest 5 years)
- [ ] Current market cap and stock price (within last trading day)
- [ ] Latest 10-K/10-Q filings for accurate working capital and capex
- [ ] Current risk-free rate (10-year Treasury yield) from web search
- [ ] Beta from recent sources (not training data)

**COMMON MISTAKE**: Using pre-2024 financials from training data as base year.
...
When building DCF models, use this pattern:
...
output_file = output_dir / f"{ticker}_DCF_Model_{date}.xlsx"
```
...
Example:
```
✅ 任务完成，文件已生成：CATL_DCF_Model_2026-03-06.xlsx
```
...
**CRITICAL**: 无论中间经历什么限流、fallback、SSE抖动，只要DCF模型最终生成，必须显式发送完成信号：

```
[STATUS: DCF_TASK_COMPLETED]
[ARTIFACT: <文件名>.xlsx]
[VALIDATION: SUCCESS]
```
...
**规则：**
1. 只要 `.xlsx` 文件成功写入磁盘，就发送完成信号
2. 即使前面有 API 限流、模型 fallback、搜索失败，只要最终文件生成，就必须发送
3. 信号放在返回内容的**最后**，确保前端能收到
4. 如果文件生成失败，发送：`[STATUS: DCF_TASK_FAILED]` 并说明原因
...
print(f"✅ DCF model completed: {len(completed_files)} files generated")
