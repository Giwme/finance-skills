...
**BEFORE STARTING:**
1. **Check today's date** - All comps must use current financial data
2. **MCP sources preferred** - If S&P Kensho, FactSet, or Daloopa MCP available, use exclusively
3. **FREE ALTERNATIVES (No MCP required)**:
   - **Brave API + web_search** ✅ - 推荐，已配置
   - **SEC EDGAR** - 免费获取 10-K/10-Q 财报
   - **Yahoo Finance API** - 免费获取股价、市值
   - **Company investor relations pages** - 季度财报和 presentation
4. **NEVER use training data** (cutoff: early 2024) for revenue, EBITDA, or multiples

**Data Freshness Requirements:**
- [ ] All company financials: **最新季度优先**，其次 FY2025（explicitly dated）
- [ ] Valuation multiples: current market data (within last 3 months)
- [ ] Stock prices: latest trading day
- [ ] Source citations: URL or database reference for every number

**Why this matters:** Training data is outdated. MCP sources provide verified, institutional-grade data with proper audit trails.
...
output_file = output_dir / f"{company}_Comps_Analysis_{date}.xlsx"
```
...
Example:
```
✅ 任务完成，文件已生成：AAPL_Comps_Analysis_2026-03-06.xlsx
```
...
无论中间经历什么限流、fallback、SSE抖动，只要最终输出文件生成，必须显式发送完成信号：
...
print(f"✅ Comps analysis completed: {len(completed_files)} files generated")
